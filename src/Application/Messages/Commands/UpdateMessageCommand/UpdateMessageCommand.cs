...
public record UpdateMessageCommand(int messageId, string Content, DateTimeOffset Created)
    : BaseModifyMessageCommand(Content, Created), IRequest;

public class UpdateMessageCommandHandler(IIdentityService identityService, IApplicationDbContext dbContext)
    : IRequestHandler<UpdateMessageCommand>
{
    ...
    {
        Message? message = await dbContext.Messages.FindAsync(command.messageId, cancellationToken);
